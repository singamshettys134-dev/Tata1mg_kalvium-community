...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    const pharmacy = await prisma.pharmacy.findUnique({ where: { id: params.id } });
    ...
    const fileName = `pharmacy_${params.id}_${Date.now()}${ext}`;
    const uploadDir = path.join(process.cwd(), 'public', 'uploads', 'licenses', 'pharmacies');
    await fs.mkdir(uploadDir, { recursive: true });
    await fs.writeFile(path.join(uploadDir, fileName), new Uint8Array(await file.arrayBuffer()));

    const fileUrl = `/uploads/licenses/pharmacies/${fileName}`;
    const upload = await prisma.fileUpload.create({
      data: { fileUrl, fileType: 'LICENSE', mimeType: file.type, uploadedBy: auth.user.userId },
    });

    await prisma.verificationRequest.create({
      data: { entityType: 'PHARMACY', entityId: params.id, status: 'UNDER_REVIEW', supportingDocumentId: upload.id },
    });

    await prisma.auditLog.create({
      data: { userId: auth.user.userId, action: 'UPLOAD_PHARMACY_LICENSE', details: `Uploaded license for pharmacy ${params.id}` },
    });

    return jsonSuccess({ fileUrl, fileName, fileSize: file.size, uploadId: upload.id }, 201);
  } catch (err) {
    console.error('[POST /api/admin/pharmacies/[id]/license]', err);

...
export function AuthPage({ onLogin, onBack, initialError }: AuthPageProps) {
      ...
      <div className="flex-1 flex flex-col justify-center px-6 py-10 lg:px-12 overflow-y-auto">
        ...
        <div className="w-full max-w-md mx-auto">
          ...
          <form onSubmit={handleSubmit} noValidate className="space-y-4">
            ...
            <button
              type="button"
              disabled={tab === 'signup' && !role}
              onClick={() => {
                const googleUrl = tab === 'signup' && role ? `/api/auth/google?role=${role.toUpperCase()}` : '/api/auth/google';
                window.location.href = googleUrl;
              }}
              className="w-full h-11 flex items-center justify-center gap-3 rounded-xl border-2 border-gray-200 hover:border-gray-300 hover:bg-gray-50 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
              ...
              {tab === 'signup' && !role ? 'Select a role above first' : 'Continue with Google'}
            </button>
          </form>
